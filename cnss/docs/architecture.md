...
5. **Bytes-per-Second (BPS) Aggregation**: All subscription handlers now calculate and return bytes-per-second metrics in parallel with packets-per-second (PPS). Handlers querying `telemetry_1s` use `SUM(t.bytes_in)` and `SUM(t.bytes_out)`, while handlers querying raw `packet_flows` use `SUM(size)` to compute throughput rates.
6. **Optimization**: Before executing SQL, it checks `SMEMBERS sub:listeners:{query_hash}`. If no WebSocket clients are listening, the SQL query is skipped to save database resources.
7. **Pub/Sub Publishing**: Formats the aggregated data according to the API schema and publishes it to Redis Pub/Sub on the channel `ws:push:{query_hash}`.
8. **State Synchronization & Timeout Enforcement (Slow Path)**:
...
CREATE TABLE packet_flows (
    ...
    size BIGINT NOT NULL DEFAULT 0, -- Packet size in bytes
    PRIMARY KEY (id, time)
);
