    ...
    try:
        async with pool.acquire() as conn:
            ...
            existing_user = await conn.fetchrow(
                "SELECT id, role FROM users WHERE username = $1", username
            )
            if existing_user:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CnSS Console Utility: Create a new user with role and scopes."
    )
    parser.add_argument(
        "username", 
        type=str, 
        help="The username for the new user (max 50 characters)"
    )
    parser.add_argument(
        "--role", 
        type=str, 
