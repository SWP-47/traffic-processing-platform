...
async def test_api_login() -> Optional[str]:
...
async def test_websocket_and_udp(token: str) -> None:
    ...
    async with websockets.connect(ws_uri) as ws:
        ...
        snapshot_str = snapshot_resp.decode("utf-8") if isinstance(snapshot_resp, bytes) else snapshot_resp
        print(f"✓ Initial Snapshot Received: {snapshot_str}")
        ...
        for _ in range(3):
            try:
                ...
                print(f"✓ WebSocket Update Received: {msg_str}")
