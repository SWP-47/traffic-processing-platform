...
class Poller:
    ...
    async def _tick(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(active_hashes) if active_hashes else 0} active subscription hashes from Redis."
            )
        except Exception as e:
        ...
        for query_hash in active_hashes:
            await self._process_subscription(cast(str, query_hash))
    ...
    async def _process_subscription(self, query_hash: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"[{query_hash}] Published telemetry update ({len(payload)} bytes) to {listener_count} listener(s)."
            )
