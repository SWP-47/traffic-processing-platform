...
class DropFlusher:
    ...
    async def _flush_channel(self, channel_id: str) -> None:
        ...
        try:
            ...
            is_active_in_redis = state_data.get("is_active") in ("1", 1)
            ...
            # were received, not the time the flusher executes the query.
            ...
            if last_activity_at_raw:
                try:
                    ...
                    logger.warning(
                        f"[{channel_id}] Invalid last_activity_at format in Redis: {last_activity_at_raw}"
                    )
            ...
            # - last_activity_at = $4 (use exact timestamp from Redis, avoiding NOW() drift)
                ...
                await conn.execute(
                    ...
                    SET 
                        ...
                        last_activity_at = CASE 
                            WHEN $2::boolean AND $4::timestamptz IS NOT NULL 
                            THEN $4::timestamptz 
            ...
            logger.info(
                ...
                f"reactivated={is_active_in_redis}, last_activity={last_activity_at}."
            )
