...
class TestChannelsEndpoint:
    ...
    async def test_admin_sees_all_channels(self, client, admin_headers):
        mock_db_data = [
            {"channel_id": "ch-admin-1", "last_activity_timestamp": datetime.now(timezone.utc)},
            {"channel_id": "ch-admin-2", "last_activity_timestamp": datetime.now(timezone.utc)},
        ]
        with patch("app.main.get_all_channels_from_db", new_callable=AsyncMock, return_value=mock_db_data):
    ...
    async def test_viewer_sees_only_scoped_channels(self, client, viewer_headers):
        mock_db_data = [
            {"channel_id": "bridge-berlin-01", "last_activity_timestamp": datetime.now(timezone.utc)},
            {"channel_id": "bridge-prague-01", "last_activity_timestamp": datetime.now(timezone.utc)},
        ]
        with patch("app.main.get_all_channels_from_db", new_callable=AsyncMock, return_value=mock_db_data):
...
class TestChannelStatusEndpoint:
    ...
    async def test_ac4_admin_gets_existing_channel(self, client, admin_headers):
        mock_db_data = {"channel_id": "bridge-berlin-01", "last_activity_timestamp": datetime.now(timezone.utc)}
        with patch("app.main.get_channel_status_from_db", new_callable=AsyncMock, return_value=mock_db_data):
            response = client.get("/api/v1/channel/bridge-berlin-01/status", headers=admin_headers)
            assert response.status_code == 200
            data = response.json()
    ...
    async def test_ac4_viewer_gets_scoped_channel(self, client, viewer_headers):
        mock_db_data = {"channel_id": "bridge-berlin-01", "last_activity_timestamp": datetime.now(timezone.utc)}
        with patch("app.main.get_channel_status_from_db", new_callable=AsyncMock, return_value=mock_db_data):
            response = client.get("/api/v1/channel/bridge-berlin-01/status", headers=viewer_headers)
            assert response.status_code == 200
    ...
    def test_viewer_gets_forbidden_on_unscoped_channel(self, client, viewer_headers):
        ...
        response = client.get("/api/v1/channel/bridge-prague-01/status", headers=viewer_headers)
    ...
    async def test_ac5_non_existent_channel_returns_404(self, client, admin_headers):
        with patch("app.main.get_channel_status_from_db", new_callable=AsyncMock, return_value=None):
            response = client.get("/api/v1/channel/non-existent/status", headers=admin_headers)
