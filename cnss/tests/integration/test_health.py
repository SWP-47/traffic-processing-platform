...
class TestHealthEndpoint:
    ...
    async def test_health_endpoint_returns_200(self, client, admin_headers):
        with patch("app.main.is_db_healthy", new_callable=AsyncMock, return_value=True), \
             patch("app.main.get_health_metrics_from_db", new_callable=AsyncMock, return_value={"channels_total": 2, "channels_active": 1}):
    ...
    async def test_health_returns_503_when_db_unhealthy(self, client, admin_headers):
        with patch("app.main.is_db_healthy", new_callable=AsyncMock, return_value=False), \
             patch("app.main.get_health_metrics_from_db", new_callable=AsyncMock, return_value={"channels_total": 0, "channels_active": 0}):
            response = client.get("/api/v1/health", headers=admin_headers)
