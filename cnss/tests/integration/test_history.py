...
class TestHistoryEndpoint:
        ...
        mock_points = [
            {
                ...
                "packets_out_per_sec": 20.0,
                "is_active": True,
            }
        ]
        with patch(
            "app.main.get_channel_history",
            new_callable=AsyncMock,
            return_value=(60, mock_points),
        ), patch(
            ...
            response = client.get(
                "/api/v1/channel/bridge-berlin-01/history?period=24h",
    ...
    async def test_invalid_period_returns_400(self, client, admin_headers):
        ...
        with patch(
            ...
            response = client.get(
                "/api/v1/channel/bridge-berlin-01/history?period=2d",
                headers=admin_headers,
            )
    ...
    async def test_channel_not_found_returns_404(self, client, admin_headers):
        ...
        with patch(
            ...
            response = client.get(
                "/api/v1/channel/non-existent/history?period=24h", headers=admin_headers
            )
    ...
    async def test_viewer_forbidden_returns_403(self, client, viewer_headers):
        ...
        response = client.get(
            "/api/v1/channel/bridge-rome-01/history?period=24h", headers=viewer_headers
        )
