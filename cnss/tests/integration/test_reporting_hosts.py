    ...
    with patch("app.tasks.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        ...
        with patch("app.tasks.state_store", store):
            with patch("app.tasks.get_reporting_data", new_callable=AsyncMock, return_value=(metrics, last_seen_map)):
                with patch("app.tasks.broadcast_telemetry_update", new_callable=AsyncMock):
                    with patch("app.tasks.get_top_hosts", new_callable=AsyncMock, return_value=[]) as mock_hosts_db:
                        with patch("app.tasks.broadcast_hosts_update", new_callable=AsyncMock) as mock_broadcast_hosts:
                            await reporting_worker_task()
                            return mock_hosts_db, mock_broadcast_hosts

@pytest.mark.asyncio
async def test_reporting_worker_skips_db_query_when_no_subscribers(mock_store, mock_settings):
    """
    ...
    mock_hosts_db, mock_broadcast_hosts = await run_reporting_iteration(mock_store, last_seen_map)
...
@pytest.mark.asyncio
async def test_reporting_worker_queries_and_broadcasts_when_subscribers_exist(mock_store, mock_settings):
    """
    ...
    mock_ws = AsyncMock()
    session = WSClientSession(websocket=mock_ws, user=MagicMock(), channel_id="ch-with-subs")
    session.subscriptions["lan_hosts"] = {"sort_by": "sent", "limit": 5}
    await mock_store.add_listener("ch-with-subs", session)
    
    mock_hosts = [{"ip": "10.0.0.1", "sent_per_sec": 100.0, "received_per_sec": 50.0, "last_seen": now.isoformat()}]
    
    with patch("app.tasks.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        mock_sleep.side_effect = [None, asyncio.CancelledError()]
        with patch("app.tasks.state_store", mock_store):
            with patch("app.tasks.get_reporting_data", new_callable=AsyncMock, return_value=({}, last_seen_map)):
                with patch("app.tasks.broadcast_telemetry_update", new_callable=AsyncMock):
                    with patch("app.tasks.get_top_hosts", new_callable=AsyncMock, return_value=mock_hosts) as mock_hosts_db:
                        with patch("app.tasks.broadcast_hosts_update", new_callable=AsyncMock) as mock_broadcast_hosts:
                            await reporting_worker_task()
