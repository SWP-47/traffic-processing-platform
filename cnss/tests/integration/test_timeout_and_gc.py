...
async def test_timeout_detection_and_isolation(mock_store, mock_settings):
    ...
    assert ch_b.is_active is True, "AC 3 Failed: Active channel should remain unaffected"
...
async def test_broadcast_on_timeout(mock_store, mock_settings):
    ...
    """
    now = datetime.now(timezone.utc)
    await mock_store.update_channel_activity("ch-timeout", 1, now - timedelta(milliseconds=6000))
...
async def test_garbage_collection(mock_store, mock_settings):
    ...
    Given a channel has is_active=false for >24 hours AND has zero WebSocket listeners, 
        When the garbage collector runs, Then the channel is removed from the in-memory registry.
    """
    now = datetime.now(timezone.utc)
    ...
    async with mock_store._lock:
        mock_store._channels["ch-gc"].last_activity_timestamp = now - timedelta(hours=25)
...
async def test_gc_not_triggered_with_listeners(mock_store, mock_settings):
    ...
    async with mock_store._lock:
        mock_store._channels["ch-gc-listeners"].last_activity_timestamp = now - timedelta(hours=25)
    ...
    assert ch is not None, "AC 4 Negative Failed: Channel with listeners should NOT be garbage collected"