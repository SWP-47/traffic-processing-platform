...
async def test_timeout_detection_broadcasts_inactive(mock_store, mock_settings):
    ...
    calls = {
        call.kwargs["channel_id"]: call.kwargs for call in mock_broadcast.call_args_list
    }

    assert (
        calls["ch-a"]["is_active"] is False
    ), "Timed out channel should broadcast as inactive"
    assert (
        calls["ch-b"]["is_active"] is True
    ), "Active channel should broadcast as active"
