...
class TestWebSocketTelemetry:
    ...
    def test_invalid_token(self, client):
        ...
        with pytest.raises(WebSocketDisconnect) as excinfo:
            with client.websocket_connect("/api/v1/ws/telemetry?token=badtoken&channel_id=test-ch"):
                pass
        assert excinfo.value.code == 4001
    ...
    def test_viewer_forbidden_scope(self, client):
        ...
        with patch("app.main.state_store", test_store):
            with pytest.raises(WebSocketDisconnect) as excinfo:
                with client.websocket_connect(f"/api/v1/ws/telemetry?token={token}&channel_id=test-ch"):
                    pass
    ...
    def test_channel_not_found(self, client):
        ...
        with patch("app.main.state_store", test_store):
            with pytest.raises(WebSocketDisconnect) as excinfo:
                with client.websocket_connect(f"/api/v1/ws/telemetry?token={token}&channel_id=non-existent"):
                    pass
    ...
    async def test_broadcast_to_multiple_listeners(self):
        ...
        batch = TelemetryBatch(
            channel_id="test-ch", sequence=1, window_ms=500,
            direction_out={"packets": 10}, direction_in={"packets": 10},
