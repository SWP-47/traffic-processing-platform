...
class TestWebSocketHosts:
    ...
    async def test_subscribe_sends_initial_snapshot(self, client):
        ...
        mock_hosts = [{"ip": "192.168.1.1", "sent_per_sec": 10.0, "received_per_sec": 5.0, "last_seen": "2026-06-17T12:00:00Z"}]
        
        with patch("app.main.state_store", test_store), \
             patch("app.main.get_top_hosts", new_callable=AsyncMock, return_value=mock_hosts):
            
            with client.websocket_connect(url) as websocket:
                # Send subscribe message
                sub_msg = {"action": "subscribe", "target": "lan_hosts", "sort_by": "sent", "limit": 5}
    ...
    async def test_unsubscribe_removes_from_session(self, client):
        ...
        url = f"/api/v1/ws/telemetry?token={token}&channel_id=test-ch"
        with patch("app.main.state_store", test_store), \
            patch("app.main.get_top_hosts", new_callable=AsyncMock, return_value=[]):
            with client.websocket_connect(url) as websocket:
                # Subscribe first
                websocket.send_text(json.dumps({"action": "subscribe", "target": "wan_hosts", "sort_by": "received", "limit": 3}))
                websocket.receive_json() # consume initial snapshot
                
                # Unsubscribe
                websocket.send_text(json.dumps({"action": "unsubscribe", "target": "wan_hosts"}))
