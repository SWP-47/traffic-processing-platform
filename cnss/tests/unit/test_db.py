...
async def test_db_fetch_user_by_username(mock_pool, mock_conn):
    mock_conn.fetchrow = AsyncMock(
        return_value={"id": "user-uuid", "username": "admin", "password_hash": "hash", "role": "admin"}
    )
...
async def test_db_list_channels_filtered(mock_pool, mock_conn):
    ...
    mock_conn.fetch.assert_called_once_with(
        "SELECT channel_id, is_active, last_activity_at FROM channels WHERE channel_id = ANY($1)", ["ch1"]
    )
        ...
        return_value=[
            {
                ...
                "packets_out_per_sec": 2.0,
                "is_active": True,
            }
        ]
    )
    history = await db_fetch_channel_history(
        "ch1", datetime.now(timezone.utc), datetime.now(timezone.utc), 10, pool=mock_pool
    )
...
async def test_db_fetch_host_history(mock_pool, mock_conn):
    mock_conn.fetch = AsyncMock(
        return_value=[{"timestamp": datetime.now(timezone.utc), "packets_in_per_sec": 1.0, "packets_out_per_sec": 2.0}]
    )
    history = await db_fetch_host_history(
        "ch1", "192.168.1.1", datetime.now(timezone.utc), datetime.now(timezone.utc), 10, pool=mock_pool
    )
    assert len(history) == 1
    assert history[0]["packets_in_per_sec"] == 1.0
...
async def test_db_fetch_host_top_destinations_data(mock_pool, mock_conn):
    ...
    res = await db_fetch_host_top_destinations_data(
        "$1", "$2", "$3", 5.0, "", "", "", ["192.168.1.5", "ch1", "5s"], pool=mock_pool
    )
    assert len(res) == 1
...
async def test_db_fetch_host_top_ports_data(mock_pool, mock_conn):
    ...
    res = await db_fetch_host_top_ports_data(
        "$1", "$2", "$3", 5.0, "", "", "", ["192.168.1.5", "ch1", "5s"], pool=mock_pool
    )
    assert len(res) == 1
