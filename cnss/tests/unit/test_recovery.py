...
async def test_recovery_broadcasts_to_listeners():
    ...
    with patch("app.udp_server.state_store") as mock_store:
        ...
        mock_store.get_listeners = AsyncMock(return_value={AsyncMock()}) # Mock listener
        
        with patch("app.udp_server.broadcast_telemetry_update", new_callable=AsyncMock) as mock_broadcast:
            ...
            assert is_active is True, "Broadcast must notify listeners that channel is active"