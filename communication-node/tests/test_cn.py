...
class TestMakeJsonForCnss:
    def test_correct_data_real_packets(self):
        ...
        created_json = cn_demo_1.make_json_for_cnss(
            fake_packets_to_send, fake_sequence
        )

        assert created_json["channel_id"] == cn_demo_1.CHANNEL_ID
        assert isinstance(created_json["timestamp"], int)
        assert (
            created_json["timestamp"] >= 10**9
            and created_json["timestamp"] < 10**10
        )
...
class TestProcessPacket:
    ...
    def test_payload_is_not_json(self):
        fake_pkt_payload = '"direction": 1, "src_ip": "192.168.100.1", \
            "dst_ip": "192.168.100.2", "src_port": 9090, "dst_port": 1547'
