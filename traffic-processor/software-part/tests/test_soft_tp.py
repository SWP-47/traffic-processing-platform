...
class TestGetJsonPayload:
    def test_ipv4_packet_ports_exist(self):
        fake_direction = 1
        fake_pkt = Ether() / IP(src="192.168.100.1", dst="192.168.100.2") / UDP(sport=9090, dport=1547)
    ...
    def test_direction(self):
        ...
        created_json_out = tp_packet_counter.get_json_payload(fake_pkt_out, fake_direction_out)

        fake_direction_in = 0
        fake_pkt_in = Ether() / ARP()
        created_json_in = tp_packet_counter.get_json_payload(fake_pkt_in, fake_direction_in)
